...
- Перед запуском workflow проверяет колонку `Result` в Google Sheets
- Если ссылка уже есть — строка пропускается
- Обрабатываются только строки с пустым полем `Result`
- Если все строки обработаны — workflow останавливается автоматически
